      ...
      >
        ...
        <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 4, padding: "0 12px" }}>
          {NAV_ITEMS.map(({ label, href, icon, danger }) => {
            ...
            const activeColor = danger ? "var(--error)" : "var(--blue-700)";
            const activeBg = danger ? "var(--error-bg)" : "var(--action-bg)";
            const hoverBg = danger ? "rgba(239,68,68,0.04)" : "rgba(37,99,235,0.05)";
            return (
              <button
                key={href}
                ...
                style={{
                  ...
                  color: active ? activeColor : danger ? "var(--error)" : "var(--text-body)",
