...
def generate_next_question(candidate_id: int) -> dict:
    with Session(engine) as session:
        ...
        # ── Per-skill confidence: seed on first call ──────────────────────────────
        skill_confidences = session.exec(
            select(SkillConfidence).where(SkillConfidence.session_id == session_row.id)
        ).all()

        if not skill_confidences and session_row.question_count == 0 and claims:
            for claim in claims:
                sc = SkillConfidence(
        ...
        # ── Pick target skill (unevaluated, fewest questions asked — breadth first) ──
        target_skill_name: str | None = None
        if skill_confidences:
            unevaluated = [sc for sc in skill_confidences if sc.status == "evaluating"]
            if not unevaluated:
                # All skills have reached 85% confidence — signal success
                target_skill_name = None
            else:
                target_sc = min(unevaluated, key=lambda sc: sc.question_count)
                target_skill_name = target_sc.skill_name
        # ─────────────────────────────────────────────────────────────────────────────
        ...
        # Build target skill block for prompt injection
        if target_skill_name:
            target_claim = next((c for c in claims if c.skill_name == target_skill_name), None)
            target_ctx = (target_claim.context or "").strip() if target_claim else ""
            target_block = (
                f"\n## Target Skill for This Question\n"
                f"Generate a question specifically testing: **{target_skill_name}**\n"
                + (f"Resume context: {target_ctx}\n" if target_ctx else "")
            )
        else:
            target_block = ""

        human_content = f"""{stage_block}{target_block}
...
Return valid JSON only."""
        ...
        # All per-skill confidences reached ≥85% — trigger success
        if skill_confidences and not any(sc.status == "evaluating" for sc in skill_confidences):
            if assessment_status == "CONTINUE":
                assessment_status = "TERMINATE_SUCCESS"
        ...
        if assessment_status in status_map:
            ...
            session_row.completed_at = datetime.now(timezone.utc).isoformat()
            session.add(session_row)
            session.commit()
            session.refresh(session_row)
            ...
            # Mark invitation as completed if candidate arrived via invite token
            if session_row.invite_token_used:
                from backend.models.tenant import Invitation
                invite = session.get(Invitation, session_row.invite_token_used)
