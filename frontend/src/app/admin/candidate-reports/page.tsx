...
function ScoreBadge({ score }: { score: number | null }) {
  ...
  const bg = pct >= 70 ? "var(--success-bg)" : pct >= 40 ? "var(--warning-bg)" : "var(--error-bg)";
  return (
...
function SectionLabel({ icon, text }: { icon: React.ReactNode; text: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 10 }}>
      <span style={{ color: "var(--action)", fontSize: 14 }}>{icon}</span>
      <span className="mono" style={{ fontSize: 11, fontWeight: 700, color: "var(--action)", textTransform: "uppercase", letterSpacing: "0.08em" }}>
        {text}
      </span>
    </div>
  );
...
function CandidateReportPanel({ candidateId }: { candidateId: number }) {
  ...
  const statusColor: Record<string, string> = {
    terminated_success: "var(--success)",
    terminated_fail: "var(--error)",
    terminated_limit: "var(--warning)",
    active: "var(--blue-700)",
  };
  const sessionStatus = session.status ?? "active";
  const confidence = session.confidence ?? 0;
  const progressColor = confidence >= 70 ? "var(--success)" : confidence >= 40 ? "var(--warning)" : "var(--error)";
                ...
                >
                  {b.repo && (
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
                      ...
                      {b.language && <Tag style={{ background: "var(--purple-bg)", border: "none", color: "var(--purple)", borderRadius: 999, fontSize: 11, fontFamily: "monospace" }}>{b.language}</Tag>}
                      {b.claim && <Tag style={{ background: "rgba(37,99,235,0.08)", border: "none", color: "#1d4ed8", borderRadius: 999, fontSize: 11 }}>{b.claim}</Tag>}
                      {b.alignment && (
                        <Tag
                          style={{
                            background: b.alignment === "strong" ? "var(--success-bg)" : b.alignment === "partial" ? "var(--warning-bg)" : "var(--error-bg)",
                            border: "none",
                            color: b.alignment === "strong" ? "var(--success)" : b.alignment === "partial" ? "var(--warning)" : "var(--error)",
                            borderRadius: 999,
                            fontSize: 11,
                            fontWeight: 700,
                          }}
                  ...
                  {b.redFlags && b.redFlags !== "None" && (
                    <div>
                      <span className="mono" style={{ fontSize: 10, color: "var(--error)", textTransform: "uppercase", letterSpacing: "0.06em" }}>
                        <WarningOutlined style={{ marginRight: 4 }} />Red Flags
                      </span>
                      <p style={{ margin: "3px 0 0", fontSize: 13, color: "var(--error)", lineHeight: 1.5 }}>{b.redFlags}</p>
                    </div>
                  )}
                  {!b.repo && (
            ...
            {assessments.map((a, i) => (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                  ...
                  <Tag style={{ background: a.source === "github" ? "var(--action-bg)" : "var(--purple-bg)", border: "none", color: a.source === "github" ? "var(--blue-700)" : "var(--purple)", borderRadius: 999, fontSize: 10 }}>
